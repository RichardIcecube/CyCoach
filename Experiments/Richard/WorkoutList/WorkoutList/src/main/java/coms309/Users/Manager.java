...
public class Manager extends Person{
    HashMap<Coach, HashMap<String, Athlete>> assignmentList = new HashMap<>();
    ...
    public @ResponseBody String createUser(@PathVariable String userClass, @RequestBody Person person)
    {
        switch(userClass)
        {
            case "Coach":
                ...
                HashMap<String, Athlete> temp = new HashMap<>();
                assignmentList.put((Coach)person, temp);
                break;
            case "Athlete":
    ...
    {
        assignmentList.get((Coach)userList.get(coachUserName)).put(athleteUserName,
                (Athlete)userList.get(athleteUserName));
        ((Athlete) userList.get(athleteUserName)).setCoachUserName(coachUserName);
        ((Coach)userList.get(coachUserName)).setAthleteList(assignmentList.get((Coach)userList.get(coachUserName)));
    ...
    public @ResponseBody HashMap<Coach, HashMap<String, Athlete>> getAssignmentList() { return assignmentList; }
    @DeleteMapping("/manager/roster/{userName}")
    public @ResponseBody HashMap<String, Person> deleteUser(@PathVariable String userName)
    {
        if(assignmentList.containsKey((Coach)userList.get(userName)))
        {
            assignmentList.remove((Coach)userList.get(userName));
            userList.remove(userName);
            return userList;
        }
        if(assignmentList.containsValue((Athlete)userList.get(userName)))
        {
            Coach c = (Coach)userList.get(((Athlete)userList.get(userName)).getCoachUserName());
            assignmentList.get(c).remove(userName);
            userList.remove(userName);
            return userList;
        }
    ...
    public @ResponseBody Person updateUser(@PathVariable String userName, @RequestBody Person person)
    {
        ...
        {
            Person p = userList.get(userName);
            p.setDateJoined(person.getDateJoined());
